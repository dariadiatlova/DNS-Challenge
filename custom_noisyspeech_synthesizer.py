...
def _get_txt_filename(wav_filename: str) -> str:
    """
    ...
    """
    pattern = re.compile(r"(\d*\w*)\.wav")
    found = pattern.search(wav_filename)
    txt_filename = found.group(1) + ".txt"
    return txt_filename


def _gen_new_txt(txt_filenames: List[str], directory_path: str):
    """
    Function takes list of txt filenames that are then read and txt from them concatenated and returned as a 1 str.
    :param txt_filenames:
    :return: str
    """
    new_txt = []
...
def gen_new_audio(filenames: List[str], params: Dict):
    """
    ...
    for audiopath in filenames:
        ...
        if remaining_audio_length > 0:
            ...
            if len(input_audio) < remaining_audio_length:
                txt_files_to_add.append(_get_txt_filename(audiopath))
    ...
    new_txt = _gen_new_txt(txt_files_to_add, params["transcripts_dir"])
