...
def build_audio(is_clean, params, index):
    """
    ...
    output_audio = np.zeros(audio_samples_length)

    if is_clean:
        source_files = params['cleanfilenames']
        idx = index
    else:
        raise Exception("Unexpected param: is_clean is False!")
    ...
    # if current file is longer than target len - pass
    if len(input_audio) > audio_samples_length:
        sys.stderr.write("WARNING: Skipped file as it's too long: %s\n" % source_files[idx])
        return

    pad_size = (len(output_audio) - len(input_audio)) // 2
    output_audio[pad_size:len(input_audio)] = input_audio

    return output_audio, idx
...
def gen_audio(is_clean, params, index, audio_samples_length=-1):
    ...
    if is_clean:
        source_files = params['cleanfilenames']
        audio, index = build_audio(is_clean, params, index)
        return audio, index, [source_files[index]]
...
def gen_noise(is_clean, params, index, audio_samples_length=-1):
    '''Calls build_audio() to get an audio signal, and verify that it meets the
       activity threshold'''
...
def main_gen(params):
    ...
    while file_num <= params['fileindex_end']:
        ...
        clean,  clean_index, clean_sf = gen_audio(True, params, clean_index)
    ...
    return clean_source_files, noise_source_files, noise_clipped_files, noise_low_activity_files
...
def main_body():
    ...
    clean_source_files, noise_source_files, noise_clipped_files, noise_low_activity_files = main_gen(params)
    ...
    total_clean = len(clean_source_files)
    ...
    print("Of the " + str(total_clean) + " clean speech files analyzed, " + "% active percentage)")
