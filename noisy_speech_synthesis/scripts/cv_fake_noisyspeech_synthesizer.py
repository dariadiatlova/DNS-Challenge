...
def main_gen(params: Dict):
    ...
    for j in trange(files_to_generate):
        ...
        _write_txt(new_txt, params["transcripts_destination"] + f"CV_{j}_clean.txt")
        ...
        audiowrite(params["clean_destination"] + f"/CV_{j}_clean.wav", clean_audio)
        audiowrite(params["fake_noisy_destination"] + f"/CV_{j}_clean.wav", clean_audio)
    return
